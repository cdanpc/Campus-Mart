...
export default function ReceiptModal({ isOpen, onClose, orderData }) {
  ...
  const handleDownload = () => {
    ...
    console.log('Download receipt for order:', orderData?.order_id);
  ...
  const orderDate = orderData?.order_date ?? orderData?.created_at;
  ...
  const buyer = {
    ...
    phone: orderData?.buyer?.phone_number ?? orderData?.contact_number ?? '—'
  };

  const seller = {
    firstName: orderData?.seller?.first_name ?? '—',
    lastName: orderData?.seller?.last_name ?? '',
    email: orderData?.seller?.email ?? '—',
    phone: orderData?.seller?.phone_number ?? '—'
  };

  const product = {
    name: orderData?.product?.name ?? 'Unknown Product',
    image: orderData?.product?.image ?? 'https://placehold.co/80x80/e5e7eb/6b7280?text=No+Image',
    description: orderData?.product?.description ?? '',
    category: orderData?.product?.category ?? '—'
  };

  const totalAmount = orderData?.total_price ?? orderData?.total_amount ?? 0;
